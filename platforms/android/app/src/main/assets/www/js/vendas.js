...
document.addEventListener("deviceready", function() {
    ...
    vendas.addEventListener('click', function(event) {
        ...
        if (verificaClient() && verificaValor() && verificaPagamento() && verificaQuantidade()) {
            let date = new Date();     
            db.transaction(function(tx) {
                tx.executeSql("UPDATE gas set quantidade = quantidade - ? where id_adm = ?", [verificaQuantidade(), '1'],
                function(tx, result) {
                    tx.executeSql("INSERT INTO vendasTotal (client, data, valor, pago, quantidade, tipo, id_adm) VALUES (?, ?, ?, ?, ?, ?, ?)", 
                        [verificaClient(), date, verificaValor(), verificaPagamento(), verificaQuantidade(), 'gas', '1'], function(tx, res) {
