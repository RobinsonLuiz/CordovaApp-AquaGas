...
document.addEventListener("deviceready", function () {
  ...
  document
    .querySelector(".btn-atualizar")
    .addEventListener("click", function () {
      db.transaction(function (tx) {
        tx.executeSql(
          "UPDATE agua set quantidade = ? where id_adm = ?",
          [document.querySelector("#agua").value, "1"],
          function (tx, result) {
            tx.executeSql(
              "UPDATE gas set quantidade = ? where id_adm = ?",
              [document.querySelector("#gas").value, "1"],
              function (tx, result) {
